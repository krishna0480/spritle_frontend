  ...
  return (
    <Box maxW="5xl" mx="auto" py="8" px="4">
      <HStack justify="space-between" mb="8">
        ...
        <Button size="sm" variant="outline" onClick={fetchLogs} borderRadius="lg" leftIcon={<RefreshCw size={14} />}>
          Refresh
        </Button>
      </HStack>
      ...
      {logs.length === 0 && !error && (
        <Box p="16" textAlign="center" bg="white" borderRadius="2xl" border="1px dashed" borderColor="gray.200">
          <Box mb="4" display="flex" justifyContent="center"><Radio size={32} color="#94a3b8" /></Box>
          <Text fontWeight="700" color="gray.400" mb="2">No webhook events yet</Text>
          <Text fontSize="sm" color="gray.400">
            Configure a webhook in Freshdesk pointing to{" "}
            <Code fontSize="xs" px="2" py="0.5" borderRadius="md">/webhook/freshdesk</Code>
          </Text>
        </Box>
      )}
        ...
        {logs.map((log) => {
          ...
          return (
            <Box key={log.id} bg="white" border="1px solid"
              ...
              <HStack p="4" cursor="pointer" onClick={() => setExpanded(isOpen ? null : log.id)}
                _hover={{ bg: "gray.50" }} justify="space-between">
                <HStack spacing="3" flexWrap="wrap">
                  <Badge colorScheme={EVENT_COLOR[log.eventType] ?? "gray"} borderRadius="full" px="2.5" fontSize="xs">
                    {log.eventType}
                  </Badge>
                  <Text fontSize="xs" color="gray.400" fontFamily="mono">
                    {new Date(log.receivedAt).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" })}
                  </Text>
                </HStack>
                <Box color="gray.400">
              ...
              <Collapse in={isOpen} animateOpacity>
                <Box borderTop="1px solid" borderColor="gray.100" p="4">
                  <Text fontSize="xs" fontWeight="700" color="gray.500"
                    textTransform="uppercase" letterSpacing="0.1em" mb="2">Payload</Text>
                  <Box bg="gray.900" borderRadius="xl" p="4" overflowX="auto" maxH="400px" overflowY="auto">
